...
var fetch$1 = require('../fetch.cjs');
var dsl = require('../filters/dsl.cjs');
var lists = require('../lists.cjs');
var request = require('../request.cjs');
var resources = require('../resources.cjs');
var cosmetic = require('./bucket/cosmetic.cjs');
var network = require('./bucket/network.cjs');
var metadata = require('./metadata.cjs');
var preprocessor$1 = require('../preprocessor.cjs');
var preprocessor = require('./bucket/preprocessor.cjs');
...
class FilterEngine extends events.EventEmitter {
    ...
    static fromLists(fetch, urls, config = {}, caching) {
        return this.fromCached(() => {
            const listsPromises = fetch$1.fetchLists(fetch, urls);
            const resourcesPromise = fetch$1.fetchResources(fetch);
    ...
    static fromPrebuiltAdsOnly(fetchImpl = fetch, caching) {
        return this.fromLists(fetchImpl, fetch$1.adsLists, {}, caching);
    }
    /**
    ...
    static fromPrebuiltAdsAndTracking(fetchImpl = fetch, caching) {
        return this.fromLists(fetchImpl, fetch$1.adsAndTrackingLists, {}, caching);
    }
    /**
    ...
    static fromPrebuiltFull(fetchImpl = fetch, caching) {
        return this.fromLists(fetchImpl, fetch$1.fullLists, {}, caching);
    }
    static fromTrackerDB(rawJsonDump, options = {}) {
        const config$1 = new config(options);
        const metadata$1 = new metadata.Metadata(rawJsonDump);
        const filters = [];
        for (const pattern of metadata$1.getPatterns()) {
            filters.push(...pattern.filters);
        }
        const engine = this.parse(filters.join('\n'), config$1);
        engine.metadata = metadata$1;
        return engine;
    }
    static parse(filters, options = {}) {
        const config$1 = new config(options);
        return new this(Object.assign(Object.assign({}, lists.parseFilters(filters, config$1)), { config: config$1 }));
    }
    static deserialize(serialized) {
        const buffer = dataView.StaticDataView.fromUint8Array(serialized, {
        ...
        const config$1 = config.deserialize(buffer);
        // Optionally turn compression ON
        if (config$1.enableCompression) {
        ...
        const engine = new this({ config: config$1 });
        ...
        engine.preprocessors = preprocessor.deserialize(buffer);
        // Deserialize buckets
        engine.importants = network.deserialize(buffer, config$1);
        engine.redirects = network.deserialize(buffer, config$1);
        engine.filters = network.deserialize(buffer, config$1);
        engine.exceptions = network.deserialize(buffer, config$1);
        engine.csp = network.deserialize(buffer, config$1);
        engine.cosmetics = cosmetic.default.deserialize(buffer, config$1);
        engine.hideExceptions = network.deserialize(buffer, config$1);
        // Optionally deserialize metadata
        const hasMetadata = buffer.getBool();
        if (hasMetadata) {
            engine.metadata = metadata.Metadata.deserialize(buffer);
        }
        buffer.seekZero();
        return engine;
    }
    ...
    cosmeticFilters = [], networkFilters = [], preprocessors = [], config: config$1 = new config(), lists = new Map(), } = {}) {
        super(); // init super-class EventEmitter
        this.config = new config(config$1);
        // Subscription management: disabled by default
        this.lists = lists;
        // Preprocessors
        this.preprocessors = new preprocessor({});
        // $csp=
        this.csp = new network({ config: this.config });
        // $elemhide
        // $generichide
        // $specifichide
        this.hideExceptions = new network({ config: this.config });
        // @@filter
        this.exceptions = new network({ config: this.config });
        // $important
        this.importants = new network({ config: this.config });
        // $redirect
        this.redirects = new network({ config: this.config });
        // All other filters
        this.filters = new network({ config: this.config });
        // Cosmetic filters
        this.cosmetics = new cosmetic.default({ config: this.config });
        // Injections
     ...
     */
    getSerializedSize() {
        let estimatedSize = dataView.sizeOfByte() + // engine version
        ...
        for (const [name, checksum] of this.lists) {
            estimatedSize += dataView.sizeOfASCII(name) + dataView.sizeOfASCII(checksum);
        }
        estimatedSize += dataView.sizeOfBool();
     ...
     */
    serialize(array) {
        const buffer = dataView.StaticDataView.fromUint8Array(array || new Uint8Array(this.getSerializedSize()), this.config);
     ...
     */
    update({ newNetworkFilters = [], newCosmeticFilters = [], newPreprocessors = [], removedCosmeticFilters = [], removedNetworkFilters = [], removedPreprocessors = [], }, env = new preprocessor$1.Env()) {
        let updated = false;
    ...
    updateFromDiff({ added, removed, preprocessors }, env) {
        ...
        if (removed !== undefined && removed.length !== 0) {
            const { networkFilters, cosmeticFilters } = lists.parseFilters(removed.join('\n'), this.config);
        ...
        if (added !== undefined && added.length !== 0) {
            const { networkFilters, cosmeticFilters } = lists.parseFilters(added.join('\n'), this.config);
        ...
        if (preprocessors !== undefined) {
            for (const [condition, details] of Object.entries(preprocessors)) {
                if (details.removed !== undefined && details.removed.length !== 0) {
                    const { networkFilters, cosmeticFilters } = lists.parseFilters(details.removed.join('\n'), this.config);
                    const filterIDs = new Set([]
                        .concat(cosmeticFilters.map((filter) => filter.getId()))
                        .concat(networkFilters.map((filter) => filter.getId())));
                    Array.prototype.push.apply(removedCosmeticFilters, cosmeticFilters);
                    Array.prototype.push.apply(removedNetworkFilters, networkFilters);
                    removedPreprocessors.push(new preprocessor$1.default({
                        condition,
                        filterIDs,
                    }));
                }
                if (details.added !== undefined && details.added.length !== 0) {
                    const { networkFilters, cosmeticFilters } = lists.parseFilters(details.added.join('\n'), this.config);
                    const filterIDs = new Set([]
                        .concat(cosmeticFilters.map((filter) => filter.getId()))
                        .concat(networkFilters.map((filter) => filter.getId())));
                    Array.prototype.push.apply(newCosmeticFilters, cosmeticFilters);
                    Array.prototype.push.apply(newNetworkFilters, networkFilters);
                    newPreprocessors.push(new preprocessor$1.default({
                        condition,
                        filterIDs,
                    }));
                }
    ...
    getBaseRules = true, getInjectionRules = true, getExtendedRules = true, getRulesFromDOM = true, getRulesFromHostname = true, }) {
        ...
        const exceptions = this.hideExceptions.matchAll(request.default.fromRawDetails({
            domain: domain || '',
            hostname,
            url,
            sourceDomain: '',
    ...
    blockScripts() {
        this.updateFromDiff({
            added: [dsl.block().scripts().redirectTo('javascript').toString()],
        });
        return this;
    }
    blockImages() {
        this.updateFromDiff({
            added: [dsl.block().images().redirectTo('png').toString()],
        });
        return this;
    }
    blockMedias() {
        this.updateFromDiff({
            added: [dsl.block().medias().redirectTo('mp4').toString()],
        });
        return this;
    }
    blockFrames() {
        this.updateFromDiff({
            added: [dsl.block().frames().redirectTo('html').toString()],
        });
        return this;
    }
    blockFonts() {
        this.updateFromDiff({
            added: [dsl.block().fonts().toString()],
        });
        return this;
    }
    blockStyles() {
        this.updateFromDiff({
            added: [dsl.block().styles().toString()],
        });
        return this;
    }
}
